...
fixedDouble calculate_RCcost(const vector<int>& S, map<int, map<int, fixedDouble>>* dFtoF, fixedDouble lam) {
    fixedDouble cost = 0;
...
pair<kMSolution, bool> recsolve(vector<int>* C, vector<int>* F, map<int, map<int, fixedDouble>>* dFtoF,
    map<int, map<int, fixedDouble>>* dAtoC, fixedDouble lam, int k, const vector<vector<int>>& G){
...
#pragma omp parallel for reduction(minKMS:result)
    for (int m : *F) {
        if (omp_get_thread_num() >= 0) {
            ...
            map<int, fixedDouble> f;
            // calculate the opening cost of each i in F
            for (int i : *F) {
                f[i] = fixedDouble(k - 1.0) * lam * (*dFtoF)[i][m];
            }
            ...
            std::cout << "m: " << m << " tempCost: " << stringValue(tempS.cost()) << " tempLP_only: " << temp_lp << endl;
